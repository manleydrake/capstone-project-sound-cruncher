...
PATH_TEST_IN_16KWAVS            = '../birdclef_data/test/wav_16khz/LIFECLEF2015_BIRDAMAZON_XC_WAV_RN17272.wav'
...
resultsFileName = "test.csv".format(os.path.split(modelPath)[-1], datetime.datetime.now().strftime('%Y-%m-%d-%H-%M'))

# running test with all the data
#for i in range(int(df.shape[0]*0.0),df.shape[0]):
#print ('{}/{}'.format(i, df.shape[0]))
result = runModelOnWavByPath(model, PATH_TEST_IN_16KWAVS)    
